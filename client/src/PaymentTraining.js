...
class PaymentTraining extends Component {
  ...
  PaymentSubmit = event => {
    ...
    if (file != '') { 
      ...
      formData.append('PaymentAmt', this.state.PaymentAmt)
      formData.append('traineID', this.state.dataPayment.traineID)
      formData.append('traineName', this.state.dataPayment.traineName)
      const config = {
        headers: {
      ...
      axios.post("http://localhost:4000/Payment", formData, config).then(res => {
        console.log(res.data)
        this.setState({
        PaymentID: res.data
      })
      })
      console.log(this.state.PaymentID._id)
      formData.append('PaymentID',this.state.PaymentID._id)
      axios.put("http://localhost:4000/TraineAdvance/update", formData, config).then(res => { })
      ...
      // console.log(data)
      axios.post("http://localhost:4000/Payment/add", data).then(res => {
         this.setState({
      ...
      let datas =
      {
        ...
        traineID: this.state.dataPayment.traineID,
        traineName: this.state.dataPayment.traineName,
        PaymentIDs: this.state.PaymentID
      }
      
      console.log(this.state.PaymentID)
       axios.put("http://localhost:4000/TraineAdvance/update1", datas).then(res => { })
                                ...
                                (
                                  this.state.dataAdvance.length > 0 ?
                                    this.state.dataAdvance.map((Payments, index) =>
                                      ...
                                      <tr key={Payments._id}>
                                        <td>{index + 1}</td>
                                        <td><img src={process.env.PUBLIC_URL + `/uploads/Training/${Payments.photo}`} width="50px" height="50px" style={{ borderRadius: '30px' }} /></td>
