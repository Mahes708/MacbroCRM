...
CompanyPaymentAdd = data =>{
  axios.get(`http://localhost:4000/CompanyAdvance/${data._id}`).then(res => {
       this.setState({
...
CompanyPaymentSubmit = event =>
{
      ...
      let data = 
          {               
              ...
              companyID : this.state.PaymentValue.companyID,
              client_name : this.state.PaymentValue.client_name               
          } 
          // console.log(this.state.PaymentValue)
          axios.put("http://localhost:4000/CompanyAdvance/update",data).then(res => {  })
          axios.post("http://localhost:4000/PaymentCompany", data).then(res => { })
          this.getAll();
}
...
PaymentView = data =>
{
  axios.get(`http://localhost:4000/PaymentCompany/companyID/${data.companyID}`).then(res => {     
     this.setState({
         dataView :res.data
     })  
 })
                ...
                <tbody>
                  ...
                  <tr key={Company._id}>                   
                      ...
                      <td>
                       ...
                       <NumberFormat value={total + Company.advanceAmt } displayType={'text'} thousandsGroupStyle="lakh" thousandSeparator={true} prefix={'₹'} />  
                     </td>
                      <td><NumberFormat value={Company.cost - (total +Company.advanceAmt) } displayType={'text'} thousandsGroupStyle="lakh" thousandSeparator={true} prefix={'₹'} /></td> 
                      <td>
                          <a href="#" onClick={() => this.CompanyPaymentAdd(Company) }  data-toggle="modal" data-target="#exampleModal"> <i className="text-success fa fa-plus" /></a>&nbsp;
                          <a href="#" onClick={() => this.PaymentView(Company) }  data-toggle="modal" data-target="#exampleModal1"> <i className="text-primary fa fa-eye" /></a>&nbsp;&nbsp;
  ...
  <div className="modal-dialog" role="document">
    <div className="modal-content">
      ...
      <div className="modal-body">
        <form  onSubmit={this.CompanyPaymentSubmit}>
          ...
          <div className="form-group">
            ...
            <input type="date" name="PaymentDate"  className="form-control" onChange={this.addCompanyPayment} id="recipient-name" defaultValue />
          </div>
          <div className="form-group">
          ...
          <div className="form-group"> 
            ...
            <input type="number" name="PaymentAmt" className="form-control"  onChange={this.addCompanyPayment}id="message-text" /><br />
  ...
  <div className="modal-dialog" role="document">
    <div className="modal-content">
      ...
      <div className="modal-body">
        ...
        <p><label>Project Type :</label> &nbsp;{this.state.dataViewMain.company_name}</p> 
