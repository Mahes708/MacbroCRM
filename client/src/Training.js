...
trainingAdvanceSubmit = event =>
{
        ...
        if (!this.state.isEdit) {
            ...
            formData.append('isEdit', this.state.isEdit)
            formData.append('traineName', this.state.dataAdvance.name)
            formData.append('advanceDate', this.state.advanceDate)
            formData.append('advanceAmt', this.state.advanceAmt)
            formData.append('traineID', this.state.dataAdvance._id)
            formData.append('course_offered', this.state.dataAdvance.course_offered)
            formData.append('fees_offered', this.state.dataAdvance.fees_offered)
            const config = {
                headers: {
            ...
            axios.post("http://localhost:4000/TraineAdvance/add", formData, config).then(res => { 
                this.getAll();  
            })
        }else{
        let data = 
...
updateTraining = data =>{       
    axios.get(`http://localhost:4000/Training/${data._id}`).then(res => {  
         this.setState({
            ...
            window.location.href=`/Addtraining/?id=${this.state.editData._id}` 
     }) 
}
                                                           ...
                                                           
                                                            {
                                                                ...
                                                                <tr key={Training._id}>
                                                                    ...
                                                                    <td>
                                                                        ...
                                                                        <a href=""  onClick={() => { this.updateTraining(Training) }} ><i className=" text-dark fa fa-pencil" aria-hidden="true"  /></a>&nbsp;
                        ...
                        <div className="modal-dialog" role="document" style={{maxWidth:"700px"}}>
                            <div className="modal-content">
                                ...
                                <div className="modal-body">
                                    ...
                                    <p> <label>GRADUATE : </label>&nbsp; {this.state.datas.Student_Graduate}</p>
                                ...
                                <div className="modal-body">
                                    <form  onSubmit={this.trainingAdvanceSubmit}>
                                        ...
                                        <div className="form-group">
                                            ...
                                            <input type="file" name="photo" className="form-control"onChange={this.Empphoto} value={this.state.file}  />
                                        </div>
                                        <div className="form-group">
                                            <label htmlFor="recipient-name" className="col-form-label">Original Amount</label>
                                            <input type="text" className="form-control" name="original_amt" id="recipient-name"  defaultValue={this.state.dataAdvance.fees_offered} disabled />
                                        </div>
                                        <div className="form-group">
